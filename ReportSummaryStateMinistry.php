...
$result = mysqli_query($connString, "SELECT * FROM `sleasofficer`WHERE PlaceOfWork  ='State Ministry' and Class='Class I'") or die("database error:". mysqli_error($connString));
$class1 = mysqli_num_rows($result); 

$result1 = mysqli_query($connString, "SELECT * FROM `sleasofficer`WHERE PlaceOfWork  ='State Ministry' and Class='Special'") or die("database error:". mysqli_error($connString));
$special = mysqli_num_rows($result1); 

$result2 = mysqli_query($connString, "SELECT * FROM `sleasofficer`WHERE PlaceOfWork  ='State Ministry' and Class='Class II' or PlaceOfWork  ='State Ministry' and Class='Class III'") or die("database error:". mysqli_error($connString));
...
$pdf->Cell(5,6,'',0,0,"L");
$pdf->Cell(5,8,'Summary of State Ministry Cadre',0,0,"L");
$pdf->Cell(5,10,'',0,1,"L");
/* Column headings */
...
$pdf->Cell($width_cell[1],10,'4',1,0,'C',false); // Second column of row 1 
...
$pdf->Cell($width_cell[4],10,4-$special,1,1,'C',false); // Fifth column of row 1 
//  First row of data is over 

//  Second row of data 
$pdf->Cell($width_cell[0],10,'Class I',1,0,'C',false); // First column of row 2 
$pdf->Cell($width_cell[1],10,'11',1,0,'C',false); // Second column of row 2
...
$pdf->Cell($width_cell[4],10,11-$class1,1,1,'C',false); // Fifth column of row 2 
//   Sedond row is over 

//  Third row of data
$pdf->Cell($width_cell[0],10,'Class II / III',1,0,'C',false); // First column of row 3
$pdf->Cell($width_cell[1],10,'10',1,0,'C',false); // Second column of row 3
...
$pdf->Cell($width_cell[4],10,10-$class2_3,1,1,'C',false); // Fifth column of row 3
