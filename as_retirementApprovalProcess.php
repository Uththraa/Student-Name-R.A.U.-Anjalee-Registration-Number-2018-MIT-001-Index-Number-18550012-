...
$employeestatus_retirement = "Retired";
...
try {
    if (!empty($id_transfer)) {
        ...
        try{
            $updatethodapprovalTransfeer = $db->updateRow(
                'UPDATE requestretirement SET Progress=?,ASApprovalStatus=?,ASDate=?
                 WHERE  	Id = ?',
    ...
    if($status1 == true){
        ...
        if (!empty($sleasofficerid_transfer) ) {  
            
                try{
                    
                    $updatetSleasWorkPlace = $db->updateRow(
                        'UPDATE sleasofficer SET CurrentStatus  = ?
                        WHERE EmpNo = ?',
                        [$employeestatus_retirement,$sleasofficerid_transfer]
                    );
