<?php
   ...
   try {
       if (!empty($id_transfer)) {
           if (!empty($hodcomment_transfer) && (!empty($hodid_transfer)) && (!empty($hoddate_transfer)) ) {
            
           try{
               $updatethodapprovalTransfeer = $db->updateRow(
                   'UPDATE requesttransfer SET HODComment =?,HODApprovalStatus =?,HODId =?,HODDate =?,Progress=?
                    WHERE  	Id = ?',
                   [$hodcomment_transfer,$HODApprovalStatus_hodApproval_transfer,$hodid_transfer,$hoddate_transfer,$progress_transfer,$id_transfer]
               );
